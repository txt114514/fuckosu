...
TRAINING_ROLE_OVERRIDES = {
    "core/app/cli.py": "模型配置、环境、坐标审计和生产训练 CLI；训练数据与阶段 runner 均由仓库内部装配。",
    "core/app/factory.py": "按模型契约、配置和设备装配正式 runtime，并构造唯一坐标变换。",
    "core/app/runtime.py": "编排 Perception → Tracking → Belief → Outcome → Decision 的有状态因果推理。",
    "core/app/training.py": "把严格配置接入 typed 参数搜索与训练门禁。",
    "conf/models.py": "定义唯一严格配置、跨字段约束以及配置摘要。",
    "state/data.py": "隔离训练样本、推理帧、训练候选和推理候选，禁止运行时读取 GT-only 字段。",
    "state/quality.py": "定义统一 DataQualityIssue 与 blocking 质量门语义。",
    "state/registry.py": "注册 canonical DTO、Enum、Protocol 和 Tensor 契约，保证单一类型 identity。",
    "core/data/segments.py": "从 canonical split manifest 构建 typed 帧、belief 和 outcome 训练数据集与 DataLoader。",
    "core/data/cache/cache.py": "以临时文件、fsync、行数和 SHA-256 校验原子发布候选缓存。",
    "core/data/calibration.py": "读取版本化标定证据并复算共享 affine 方程控制点残差。",
    "core/data/coordinates.py": "构造绑定原帧尺寸、标定身份和方程指纹的共享训练帧坐标变换。",
    "core/data/repositories/sqlite.py": "在 infrastructure 边界内读取预处理 SQLite，并返回 typed repository 记录。",
    "core/perception/runtime/runtime.py": "只从 RuntimeFrame 产生 CandidateObservation，不接触训练标签或 oracle。",
    "core/perception/training/targets.py": "通过共享 FrameCoordinateTransform 把真实标注映射为训练目标。",
    "core/tracking/tracker/tracker.py": "执行确定性 association 和轨迹生命周期管理，维持稳定 track_id。",
    "core/belief/encoder.py": "用逐 track 因果状态编码观测，输出 BeliefState。",
    "core/belief/training.py": "从 typed belief 样本构造批次、损失和运行时可复用特征。",
    "core/outcome/oracle/oracle.py": "把 canonical 点、slider 和点击序列评分封装为反事实 OutcomeOracle。",
    "core/outcome/dataset/builder.py": "确定性枚举目标与 horizon，用 OutcomeOracle 构造反事实训练样本。",
    "core/outcome/model.py": "根据 belief、动作和 horizon 预测 outcome 分布、期望得分与方差。",
    "core/outcome/training.py": "训练 OutcomeModel 并计算 NLL、Brier、ECE 和 expected-score MAE。",
    "core/decision/planner.py": "只读取 BeliefState 与 OutcomeDistribution，比较 CLICK 和 WAIT 的风险调整效用。",
    "core/evaluation/scoring.py": "提供 canonical point 与 slider 空间时间评分。",
    "core/evaluation/sequence.py": "模拟点击序列、频率限制和物件消费，并输出稳定评分结果。",
    "core/evaluation/attribution.py": "用统一规则归因未通过样本，避免准确命中被误分到错误模块。",
    "core/training/optimization.py": "从有限量化参数规格持续提出未重复参数，并在全门禁通过或明确耗尽时终止。",
    "core/training/search_state.py": "按 run、dataset 和 config identity 原子持久化可恢复搜索状态。",
    "core/training/orchestration.py": "按规格表顺序执行 Perception、Tracking、Belief、Outcome、Decision、Evaluation 门禁。",
    "core/training/curriculum_data.py": "把 BASIC、MULTI_OBJECT、COMPLEX、FULL 映射为真实且累计扩展的 segment 数据视图。",
    "core/training/gallery_artifacts.py": "按 canonical 单帧事件原子发布 production PNG，并以 manifest-last 固定分类、摘要和坐标来源。",
    "core/training/hard_example_feedback.py": "把 TRAIN canonical event 聚合为绑定 run/data/config/坐标/trial/参数的可恢复帧权重，显式隔离 validation/test。",
    "core/training/production_schedule.py": "原子保存 proposal、cohort、curriculum/rung、父 checkpoint、反馈制品和最终 history 的强状态图。",
    "core/training/production_stages.py": "在累计课程数据上执行六阶段、ASHA 增量训练和 TRAIN hard-example 加权消费。",
    "core/training/production.py": "生产训练总控；持续提案、同步 ASHA 调度、强恢复和唯一 FULL 末级 winner 复验。",
    "core/training/checkpoints.py": "事务发布模型权重、数据 identity、模型契约摘要和坐标指纹。",
    "lib/environment/env_check.py": "采集 Python、依赖、FFmpeg 与 Torch/CUDA 的统一环境报告。",
    "lib/environment/training.py": "检查严格配置要求的设备、CUDA 和共享坐标标定。",
    "lib/validation/boundaries.py": "在不可信输入和各领域 handoff 处执行单次严格校验。",
    "lib/telemetry/reporter.py": "把训练指标、资源、评估和事件写入线程安全 telemetry store。",
    "lib/telemetry/store.py": "维护带 schema_version 的快照，并原子追加 JSONL 遥测。",
    "lib/visualization/renderers.py": "把不可变 telemetry snapshot 或已校验逐帧 overlay 纯投影为 dashboard/PNG，不重新评分或归因。",
...
def render_training_index(
    ...
    lines = [
        ...
        "  -> core/data/segments.py:build_training_datasets",
        "  -> core/training/production.py:ProductionTrainer",
        "       -> core/training/curriculum_data.py (BASIC -> MULTI_OBJECT -> COMPLEX -> FULL)",
        "       -> Perception -> Tracking -> Belief -> Outcome -> Decision -> Evaluation",
        "       -> core/training/optimization.py + scheduling.py (持续提案与同 frontier ASHA)",
        "       -> core/training/hard_example_feedback.py (TRAIN-only 帧权重)",
        "       -> core/training/production_schedule.py (job/checkpoint/feedback 强恢复状态)",
        "       -> core/training/checkpoints.py (每个资源 job 事务发布，最终 winner 复验)",
        "       -> core/training/gallery_artifacts.py (按真实帧事件发布 PNG + manifest)",
        "runtime: core/app/runtime.py",
        "  -> core/perception -> tracking -> belief -> outcome -> decision",
        "side channels: lib/telemetry -> lib/visualization; core/evaluation -> gallery artifacts",
        "```",
        "",
        "## Phase 0 → Phase 11 入口",
        "",
        ...
        "| `1` | `state`, `conf`, `lib/infrastructure` | typed 契约、严格配置和持久化边界 |",
        "| `2` | `core/data` | typed dataset、cache、quality gate、repository、坐标证据 |",
        "| `3` | `core/perception` | 图像到 CandidateObservation |",
        "| `4` | `core/tracking` | association、稳定 track_id 和生命周期 |",
        "| `5` | `core/belief` | 逐 track 因果 BeliefState |",
        "| `6` | `core/outcome/oracle`, `core/outcome/dataset`, `core/evaluation` | canonical scoring 与反事实标签 |",
        "| `7` | `core/outcome/model.py`, `core/outcome/training.py` | outcome 分布、校准与训练 |",
        "| `8` | `core/decision` | CLICK/WAIT optimal stopping |",
        "| `9` | `core/training` | 六阶段门禁、curriculum/ASHA、TRAIN 难例反馈、强恢复和 checkpoint |",
        "| `10` | `lib/telemetry`, `lib/visualization` | Reporter → Store → Renderer |",
        "| `11` | `core/app`, `main.py`, `src/start/main.py` | 唯一正式 runtime、CLI 与总启动流程 |",
        "",
        "旧扁平 `app/config/contracts/...` 仅是 deprecated wrapper；新代码默认使用 conf/core/lib/state。",
        "",
        "快速查询：`python project_index/build_index.py --lookup 符号名`。",
        "",
    ]
