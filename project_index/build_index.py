...
TRAINING_ROLE_OVERRIDES = {
    ...
    "conf/settings.py": "训练配置模型与 YAML 加载；解析数据集路径、item 划分、颜色 cue、点击频率上限并校验采样和分块参数。",
    ...
    "core/memory.py": "统一 CUDA runtime policy；管理 AMP、GradScaler、channels-last、TF32、显存快照和 OOM 建议。",
    ...
    "data/patch_stream.py": "基于现有 tiling 窗口生成固定尺寸 CHW patch、padding 和含 padded 尺寸的 PatchMeta 元数据。",
    "data/color_cues.py": "从 RGB 帧派生 osu 色号、白色数字/内纹和目标相关边缘输入 cue。",
    ...
    "models/stack.py": "从 Settings 统一构建 local/global/structure/fusion/spatial head 模型栈。",
    ...
    "training/spatial_decode.py": "把 patch dense 空间预测融合为全图概率画布，并解码 Top-K 空间候选。",
    "training/spatial_targets.py": "把单帧 osu 标注按 PatchMeta 光栅化为空间多任务 dense loss target。",
    "training/spatial_trainer.py": "首版单帧空间训练循环；冻结 global、串行 patch 前向和逐 patch backward。",
