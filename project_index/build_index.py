...
TRAINING_ROLE_OVERRIDES = {
    ...
    "core/decision/generator.py": "离线空间候选缓存生成器；逐帧调用空间推理，将候选逆变换到 osu 空间并按真实 CircleSize 半径匹配 temporal target。",
    ...
    "core/optimization/error_attribution.py": "为评分图集和优化分析提供共用的 unresolved 错误域分类规则。",
    "core/optimization/scoring/__init__.py": "评分子模块门面；导出 sample/trial 级聚合入口。",
    "core/optimization/scoring/evaluator.py": "按 point-slider-v2 和 click-sequence-v1 聚合 sample/trial 级质量分；目标帧按目标数加权，防止大量空帧 no-op 虚抬试验分。",
    # 坐标链路相关职责在索引中显式标明，避免维护者只修改渲染而遗漏评分与训练标签。
    "core/optimization/scoring/run_outputs.py": "把候选缓存与决策输出统一换算到 osu 坐标评分；模型归一化坐标先还原为训练帧像素。",
    ...
    "core/optimization/parameter_search/__init__.py": "参数搜索修改子模块门面；导出 ASHA、规则式调参、课程和难例诊断入口。",
    ...
    "core/optimization/parameter_search/executor.py": "根据优化计划生成有界绝对参数 job，统一校验 checkpoint 继承、短 trial ID 和 JSONL/SQLite 记录。",
    "core/optimization/parameter_search/objectives.py": "计算 quality/VRAM/latency 多目标排序值和可复现 sort key。",
    "core/optimization/parameter_search/planner.py": "根据评分、归因、历史 trial、资源指标和多目标分数生成下一轮参数调整计划。",
    "core/training_ramp.py": "执行受控渐进训练；严格样本门禁未通过时按配置消费下一个优化 job，并持久化待执行/耗尽状态。",
    "state/candidate_cache_schema.py": "定义当前候选缓存协议版本及仅供历史诊断读取的兼容版本集合。",
    ...
    "core/temporal/dataset.py": "读取当前候选缓存 JSONL，拒绝将历史固定半径标签用于训练，并按 sample_key 生成时序窗口、mask 和动作监督。",
    ...
    "core/dataset_import/loader.py": "把配置映射为 SegmentFrameDataset 与 PyTorch DataLoader，并为每条记录注入完整坐标变换规格。",
    ...
    "lib/coordinates.py": "统一解析配置、样本与预处理元数据中的 osu 到训练帧坐标变换。",
    "lib/data/dataset.py": "按片段帧索引解码原分辨率 RGB Tensor、可变长度标签、difficulty 参数及记录级坐标变换。",
    ...
    "lib/visualization/render.py": "把帧 Tensor、osu 标签和样本携带的共享坐标变换渲染为标注图片。",
    ...
    "lib/models/smet.py": "动态 top-k 稀疏线性层和普通/稀疏 Linear 工厂。",
    "lib/models/temporal_model.py": "因果 GRU 时序模型；提供 initial_state、step 流式接口和可选 SMET 稀疏 heads。",
    ...
    "lib/training/spatial_targets.py": "用统一轴对齐/仿射坐标契约把单帧 osu 标注按 PatchMeta 光栅化为空间多任务 dense loss target。",
    # 方程指纹同时覆盖坐标配置与训练帧尺寸，是缓存和继承兼容性的组成部分。
    "state/versioning.py": "生成数据、配置与完整坐标方程版本指纹，并检查缓存/继承兼容性。",
    "state/run_state.py": "保存 trial、课程阶段、rung、预算和全局步数的运行状态。",
    "state/checkpoint_schema.py": "检查点 lineage 与模型/优化器/scheduler/AMP 恢复契约。",
    "state/experiment_schema.py": "三层参数、规则式搜索、历史搜索枚举、ASHA trial、课程和独立评估契约。",
