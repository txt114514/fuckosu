...
TRAINING_ROLE_OVERRIDES = {
    ...
    "core/pipeline.py": "声明训练阶段注册表；当前登记 dataset_import，后续扩展空间、候选缓存、时序和导出阶段。",
    "core/spatial/__init__.py": "空间阶段兼容门面；导出单帧推理、空间训练、候选和 slider path 解码入口。",
    "core/spatial_training/__init__.py": "空间模块训练门面；导出单帧推理、空间训练和候选字典化入口。",
    "core/spatial_training/spatial_inference.py": "单帧空间推理处理器；显式分离 GPU 前向与 CPU 画布融合、候选解码和输出缓存。",
    "core/spatial_training/spatial_trainer.py": "首版单帧空间训练循环；冻结 global、串行 patch 前向和逐 patch backward。",
    "core/candidate_cache/__init__.py": "候选缓存阶段门面；导出离线候选缓存生成器和文件契约。",
    "core/candidate_cache/generator.py": "离线空间候选缓存生成器；逐帧调用空间推理并写 JSONL/manifest 供时序模型读取。",
    ...
    "Lib/compat/legacy_imports.py": "集中登记旧训练模块路径到新 Lib/core/environment 路径的兼容别名。",
    "Lib/compat/training.py": "旧 traning.training 聚合入口的兼容导出；真实实现位于 Lib.training 与 core.spatial_training。",
