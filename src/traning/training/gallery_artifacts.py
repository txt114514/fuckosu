...
from traning.infrastructure import atomic_write_json, sha256_file
from traning.visualization import build_gallery_frame_overlay, render_gallery_png
...
def trial_gallery_directory(run_dir: Path, trial_index: int) -> Path:
    """返回与 trial checkpoint 同级且不会跨 trial 覆盖的 gallery 目录。"""
    ...
    return run_dir / "trials" / f"trial-{trial_index:06d}" / "gallery"
