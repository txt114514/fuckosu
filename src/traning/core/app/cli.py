...
from traning.conf import V2Config, load_v2_config, v2_config_to_dict
from traning.core.data import (
    audit_affine_calibration,
    build_training_datasets,
    load_affine_calibration_evidence,
)
from traning.core.training import ProductionTrainer, SearchExhaustedError
...
def env_check(
    ...
    try:
        ...
        raise typer.Exit(1) from exc
    require_cuda = (
        checked.runtime.device.value == "cuda" or checked.runtime.require_cuda
    )
    host_report = collect_environment_report(cpu_mode_allowed=not require_cuda)
    ok = report.ok and host_report.ready(require_cuda=require_cuda)
    payload = {
        "ok": ok,
        "environment": host_report.as_dict(),
        "results": tuple(
            {
                "name": item.name,
