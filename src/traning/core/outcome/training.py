...
from traning.core.outcome.dataset.builder import CounterfactualOutcomeDataset
...
def compute_outcome_loss(
    ...
    *,
    ...
    """以统一样本权重归约三个分量；expected score 仅作辅助回归。

    ``sample_weights=None`` 保留原始 PyTorch mean reduction。显式权重必须与
    ``OutcomeBatch`` 的浮点 dtype、设备和 batch 维严格一致，并以权重和归一化，
    从而只改变样本相对贡献而不随权重整体缩放改变学习率语义。
    """
    ...
    checked_sample_weights = _validate_sample_weights(sample_weights, batch)
    if checked_sample_weights is None:
        # 保持未启用 hard-example 时已有数值和 reduction 实现完全不变。
        ...
        category = _normalized_weighted_mean(
            F.cross_entropy(
                output.category_logits,
                batch.category_targets,
                reduction="none",
            ),
            checked_sample_weights,
        )
        expiry = _normalized_weighted_mean(
        ...
        score = _normalized_weighted_mean(
            F.smooth_l1_loss(
...
def _validate_sample_weights(
    ...
    """严格校验 hard-example 权重，不做隐式 reshape、搬运或 dtype 转换。"""

    if sample_weights is None:
        return None
    if not isinstance(sample_weights, torch.Tensor):
        raise TypeError("sample_weights 必须是 torch.Tensor 或 None")
    expected_shape = (len(batch.sample_ids),)
    if sample_weights.shape != expected_shape:
        raise ValueError(f"sample_weights 必须采用 [batch]，期望 {expected_shape}")
    if sample_weights.device != batch.belief_embeddings.device:
        raise ValueError("sample_weights 必须与 OutcomeBatch 位于同一设备")
    if sample_weights.dtype != batch.belief_embeddings.dtype:
        raise TypeError("sample_weights dtype 必须与 OutcomeBatch 浮点 dtype 一致")
    if not bool(torch.isfinite(sample_weights).all().item()):
        raise ValueError("sample_weights 必须全部有限")
    if bool((sample_weights <= 0).any().item()):
        raise ValueError("sample_weights 必须全部严格大于 0")
    weight_sum = sample_weights.sum()
    if not bool(torch.isfinite(weight_sum).item()):
        raise ValueError("sample_weights 权重和必须有限")
...
def _normalized_weighted_mean(
    ...
    """按同一有限正权重计算归一化均值，保持 loss 的标量契约。"""

    if per_record_loss.shape != sample_weights.shape:  # pragma: no cover - 内部不变量
        raise RuntimeError("per-record loss 与 sample_weights 形状不一致")
    return (per_record_loss * sample_weights).sum() / sample_weights.sum()
