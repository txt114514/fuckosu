...
class DataConfig:
    ...
    split_manifest: Path = Path("training_package/splits/dataset_split_manifest.json")
...
@dataclass(frozen=True, slots=True)
class AshaRungConfig:
    """一个 ASHA rung 的累计资源预算和晋级比例。"""
    ...
    def __post_init__(self) -> None:
        _require_int(
            "optimization.asha_rungs.budget_steps", self.budget_steps, minimum=1
        )
        _require_real(
            "optimization.asha_rungs.promotion_fraction",
            self.promotion_fraction,
            minimum=0.0,
            maximum=1.0,
        )
        if self.promotion_fraction <= 0.0:
            raise ValueError("optimization.asha_rungs.promotion_fraction 必须大于 0")


@dataclass(frozen=True, slots=True)
class OptimizationConfig:
    """严格验收搜索、ASHA 资源分配和难例权重的生产配置。"""

    max_trials: int | None = None
    cohort_size: int = 2
    asha_rungs: tuple[AshaRungConfig, ...] = (
        AshaRungConfig(budget_steps=1, promotion_fraction=0.5),
        AshaRungConfig(budget_steps=4, promotion_fraction=0.5),
    )
    hard_example_bonus: float = 1.0
    hard_example_max_weight: float = 4.0

    def __post_init__(self) -> None:
        if self.max_trials is not None:
            _require_int("optimization.max_trials", self.max_trials, minimum=1)
        _require_int("optimization.cohort_size", self.cohort_size, minimum=2)
        if not isinstance(self.asha_rungs, tuple) or not self.asha_rungs:
            raise TypeError("optimization.asha_rungs 必须是非空 AshaRungConfig 元组")
        if any(not isinstance(rung, AshaRungConfig) for rung in self.asha_rungs):
            raise TypeError("optimization.asha_rungs 只能包含 AshaRungConfig")
        budgets = tuple(rung.budget_steps for rung in self.asha_rungs)
        if budgets != tuple(sorted(set(budgets))):
            raise ValueError("optimization.asha_rungs budget_steps 必须严格递增")
        _require_real(
            "optimization.hard_example_bonus",
            self.hard_example_bonus,
            minimum=0.0,
        )
        if self.hard_example_bonus <= 0.0:
            raise ValueError("optimization.hard_example_bonus 必须大于 0")
        _require_real(
            "optimization.hard_example_max_weight",
            self.hard_example_max_weight,
            minimum=1.0,
        )
        if self.hard_example_max_weight <= 1.0:
            raise ValueError("optimization.hard_example_max_weight 必须大于 1")
...
def _asha_rungs(value: object) -> tuple[AshaRungConfig, ...]:
    """严格解析 ASHA rung 数组，不接受缺失字段或未知调度键。"""

    if isinstance(value, (str, bytes)) or not isinstance(value, Sequence):
        raise TypeError("optimization.asha_rungs 必须是对象数组")
    allowed = {field.name for field in fields(AshaRungConfig)}
    result: list[AshaRungConfig] = []
    for index, item in enumerate(value):
        raw = _mapping(f"optimization.asha_rungs[{index}]", item)
        actual = set(raw)
        if actual != allowed:
            missing = ", ".join(sorted(allowed - actual))
            unknown = ", ".join(sorted(actual - allowed))
            details = []
            if missing:
                details.append(f"缺少键: {missing}")
            if unknown:
                details.append(f"未知键: {unknown}")
            raise ValueError(
                f"optimization.asha_rungs[{index}] 字段不匹配（{'；'.join(details)}）"
            )
        result.append(
            AshaRungConfig(
                budget_steps=_require_int(
                    f"optimization.asha_rungs[{index}].budget_steps",
                    raw["budget_steps"],
                    minimum=1,
                ),
                promotion_fraction=_require_real(
                    f"optimization.asha_rungs[{index}].promotion_fraction",
                    raw["promotion_fraction"],
                    minimum=0.0,
                    maximum=1.0,
                ),
            )
...
def _parse_optimization(root: Mapping[str, object]) -> OptimizationConfig:
    """解析持续 trial 搜索、ASHA 预算与 hard-example 权重。"""
    ...
    return OptimizationConfig(
        ...
        cohort_size=_require_int(
            "optimization.cohort_size",
            _value(raw, "cohort_size", defaults.cohort_size),
            minimum=2,
        ),
        asha_rungs=(
            defaults.asha_rungs
            if "asha_rungs" not in raw
            else _asha_rungs(raw["asha_rungs"])
        ),
        hard_example_bonus=_require_real(
            "optimization.hard_example_bonus",
            _value(raw, "hard_example_bonus", defaults.hard_example_bonus),
            minimum=0.0,
        ),
        hard_example_max_weight=_require_real(
            "optimization.hard_example_max_weight",
            _value(
                raw,
                "hard_example_max_weight",
                defaults.hard_example_max_weight,
            ),
            minimum=1.0,
        ),
    )
...
def v2_config_to_dict(config: V2Config) -> dict[str, object]:
    ...
    return {
        ...
        "optimization": {
            ...
            "cohort_size": config.optimization.cohort_size,
            "asha_rungs": [
                {
                    "budget_steps": rung.budget_steps,
                    "promotion_fraction": rung.promotion_fraction,
                }
                for rung in config.optimization.asha_rungs
            ],
            "hard_example_bonus": config.optimization.hard_example_bonus,
            "hard_example_max_weight": config.optimization.hard_example_max_weight,
        },
    }
