...
from traning.config import DataLoaderConfig, V2Config
...
from traning.data.coordinates import FrameCoordinateTransform
...
class SegmentTrainingDataset(Sequence[TrainingSample]):
    ...
    def __getitem__(
        ...
        if (
            ...
            raise ValueError(
                f"视频帧尺寸 {width}x{height} 与坐标标定尺寸不一致: "
                f"{record.video_path}"
            )
...
class TrainingDatasetBundle:
    ...
    def __post_init__(self) -> None:
        ...
        fingerprints = {
            dataset.transform_fingerprint for _, dataset in self.datasets
        }
...
def _dataset_identity(
    ...
    for split in _CONCRETE_SPLITS:
        for record in selected[split]:
            relative_directory = _relative_path(record.directory, config.data.dataset_root)
            row = {
                "split": split.value,
