...
class TrackedObservation:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.lifecycle, TrackLifecycle):
            raise TypeError("lifecycle 必须是 TrackLifecycle")
        ...
        if self.candidate is not None and not isinstance(
            ...
            raise TypeError("candidate 必须是 CandidateObservation")
        if self.association is AssociationStatus.CREATED:
            if self.lifecycle is not TrackLifecycle.NEW or self.candidate is None:
            ...
            if self.lifecycle is not TrackLifecycle.ACTIVE or self.candidate is None:
            ...
            if self.lifecycle not in (TrackLifecycle.MISSING, TrackLifecycle.EXPIRED):
