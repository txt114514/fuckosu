...
class ActionPrediction:
    ...
    outcome: OutcomePrediction | None = None

    def __post_init__(self) -> None:
        if not isinstance(self.action, ActionType):
            raise TypeError("action 必须是 ActionType（兼容名 DecisionAction）")
        ...
        if self.action is ActionType.CLICK:
            ...
            if not isinstance(self.outcome, OutcomePrediction):
                raise TypeError(
                    "CLICK outcome 必须是 OutcomePrediction（兼容名 OutcomeDistribution）"
                )
...
# 旧名称是 identity alias，不保留第二套动作或结果定义。
