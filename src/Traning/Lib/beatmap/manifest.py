...
from Traning.state.manifest_schema import MANIFEST_DB_FILENAME, PackageManifestItem
...
class PackageManifest:
    ...
    def _folder_name(self, number: int) -> str:
        return f"{INTERNAL_FOLDER_PREFIX}{number:06d}"
    ...
    def export_table(self) -> Path:
        temp_path = self.table_path.with_name(f".{self.table_path.name}.tmp")
