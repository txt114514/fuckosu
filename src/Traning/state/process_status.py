...
def _normalize_process_steps(process_steps: Iterable[str]) -> tuple[str, ...]:
...
def _encode_detail(detail: Any) -> str | None:
    if detail is None:
        return None
    return json.dumps(detail, ensure_ascii=False, default=str)


def _decode_detail(detail_json: str | None) -> Any:
...
class ProcessStatusManager:
    def __init__(
        ...
        self.process_steps = (
            _normalize_process_steps(process_steps)
            if process_steps is not None
            else _normalize_process_steps(load_settings().progress.process_steps or PROCESS_STEPS)
        )
    ...
    def load_status(self, folder_name: str) -> dict[str, Any]:
        ...
        with Session(self.engine) as session:
            ...
            for row in session.exec(statement):
                ...
                status["steps"][row.step]["detail"] = _decode_detail(row.detail_json)
        return status
    ...
    def save_status(self, folder_name: str, status: dict[str, Any]):
        ...
        with Session(self.engine) as session:
            for step, step_status in normalized["steps"].items():
                ...
                record.detail_json = _encode_detail(step_status["detail"])
            session.commit()
