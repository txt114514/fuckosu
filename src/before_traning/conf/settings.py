...
class SegmentSettings(BaseModel):
    ...
    def _nonnegative_interval(cls, value: int) -> int:
        if value < 0:
            raise ValueError(
                "priority_merge_window_ms must be nonnegative"
            )
        return value

    @field_validator("long_sequence_max_objects")
    @classmethod
    def _long_sequence_object_limit(cls, value: int) -> int:
        if value < 4:
            raise ValueError("long_sequence_max_objects must be at least 4")
        return value
    ...
    def _nonnegative_context(cls, value: float) -> float:
        if value < 0 or value != value or value == float("inf"):
            raise ValueError(
                "segment seconds must be finite and nonnegative"
            )
        return value

    @field_validator("long_sequence_max_duration_seconds")
    @classmethod
    def _positive_duration(cls, value: float) -> float:
        if value <= 0 or value != value or value == float("inf"):
            raise ValueError(
                "long_sequence_max_duration_seconds must be finite and positive"
            )
        return value
