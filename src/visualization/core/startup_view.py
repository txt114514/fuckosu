...
def render_startup_view(
    ...
    *,
    ...
    header = Panel(
        f"Pipeline Phase：{state.pipeline_phase}\n"
        f"当前流程：{state.phase}\n"
        f"当前状态：{state.status}\n"
    ...
    if compact:
        ...
        panels = {
            "overview": (render_pipeline_panel(state), render_resources_panel(state, compact=True)),
...
def _compact_hint(page: str, pages: tuple[str, ...]) -> str:
    ...
    return f"\n紧凑页：{current}/{len(pages)}；Tab/空格下一页，b上一页，f完整视图，Ctrl+C中断"
