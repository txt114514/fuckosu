...
- GPU 可用性验证命令：
  `host-exec docker exec -u dev osu_ai_dev bash -lc 'cd /home/dev/workspace && bash src/traning/lib/environment/check_gpu.sh'`
- 训练 CLI 的 CUDA 验证示例：
  `host-exec docker exec -u dev osu_ai_dev bash -lc 'cd /home/dev/workspace && PYTHONPATH=src python -m traning env-check --strict'`
